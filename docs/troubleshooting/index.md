...
- Shift를 누르고 있지 않은지
- `/fallingtree toggle`로 꺼 두지 않았는지
- 잎이 있는 실제 나무 형태인지
- 100블록을 넘는 거대 구조물인지
...
2. Grave Compass 확인
3. 사망 지점 주변과 차원 확인
4. 기본값에서는 자동 만료가 꺼져 있으므로 단순히 “30분 지나서 사라졌다”고 가정하지 않기
5. 서버 오류 의심 시 관리자에게 사망 시각·좌표·주요 아이템 전달
...
- UDP 음성 포트 문제는 서버 네트워크/방화벽 영역이므로 운영자에게 전달
